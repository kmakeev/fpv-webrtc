...
| `pli`  | viewer → streamer | `{}` — запрос IDR при заморозке видео (≥3 с без новых framesDecoded); bypass сетевого rate-limit 1.5 с в `webrtc_streamer.c`; обрабатывается в `datachannel.c` |
| `resolution` | viewer → streamer | `{ w, h }` — переключить разрешение камеры (применяется асинхронно в camera_task core 1); выполняется в `datachannel.c` → `camera_set_resolution()` |
...
- **Пинаут SCCB (I2C) на ESP32-P4-Function-EV-Board**: SDA=**GPIO7**, SCL=**GPIO8**, порт **I2C_NUM_0** (LP I2C). Источник: `esp-idf/examples/peripherals/camera/mipi_isp_dsi/main/example_config.h`. GPIO31/GPIO34 в логе того же примера — это I2C LCD-панели (ILI9881C), не камеры. MIPI PHY LDO: канал 3, 2500 мВ — инициализируется через `esp_ldo_acquire_channel` перед CSI.
- **CSI конфигурация**: 2 data lanes, MIPI Mbps берётся из дескриптора формата сенсора (`mipi_info.mipi_clk / 1e6`), input=RAW8 или RAW10 (по формату), output=YUV420 (ISP конвертирует). ISP: `ISP_INPUT_DATA_SOURCE_CSI`, `ISP_COLOR_RAW8` → `ISP_COLOR_YUV420`, clk=80 MHz.
- **Порядок инициализации CSI/ISP** (критично, источник: `esp-idf/examples/peripherals/camera/mipi_isp_dsi`): CSI create → register callbacks → CSI enable → ISP create → ISP enable → CSI start → sensor stream ON. Sensor stream-on должен быть ПОСЛЕДНИМ — если включить раньше CSI, MIPI-данные потекут до готовности приёмника, CSI пропустит первый VSYNC и DMA никогда не завершит транзакцию (camera_task не получит ни одного кадра).
- **Выбор формата сенсора**: `esp_cam_sensor_query_format` + score-based selection: предпочтение RAW8 (+100), малое разрешение ≤800×640 (+50), низкий fps (+5). `CONFIG_CAMERA_OV5647=y` в sdkconfig.defaults активирует OV5647 driver. OV5647 на данной плате **не имеет** формата 1280×720 — доступны: 800×640/800×800/800×1280 RAW8 и 1920×1080/1280×960 RAW10.
- **6-buffer кольцо захвата**: PSRAM буферы (64-байт alignment вручную — allocator-баг возвращает 32 байта), кольцевой индекс `g_dma_next`. Callback `on_get_new_trans` (IRAM) предоставляет следующий буфер; `on_trans_finished` (IRAM) отправляет указатель в `xQueueOverwriteFromISR`. `bk_buffer_dis=true`. **Условие NUM_CAP_BUFS**: `NUM_CAP_BUFS × frame_interval_ms > encode_time_ms`. При 1280×960@45fps: encode ~98ms, ring cycle 132ms — критически мало. При 800×640@50fps: encode ~25ms, ring cycle 120ms — безопасно.
- H.264 кодируется аппаратным VEU (`esp_h264_enc_hw_new`): битрейт масштабируется с разрешением от базовых 4 Мбит/с@1280×720, GOP = 15 (фиксированный `ENC_GOP`), QP [20, 38]. RC fps = 15 (реальный throughput HW энкодера ~12 fps при 800×640, encode avg=81ms). При fps=50 (сенсор) RC выделял 22Kbit/кадр — P-кадры были 2–4KB; при fps=15 P-кадры 6–44KB.
- **ISP pipeline**: AWB (continuous, callback `s_awb_stats_done` в IRAM), AE (continuous), Sharpen (h=2.0/m=1.5, thresh 30/5), Color (contrast=1.5, saturation=2.0, brightness=-15), Gamma 1.3 (16-точечная кривая по R/G/B). Все модули — zero cost для encode pipeline (хардверный ISP).
- **PLI/FIR rate-limit**: `on_keyframe_request_cb` в `webrtc_streamer.c` пропускает запросы чаще 1.5 с. IDR retry timer (5×2с) работает независимо — вызывает `camera_request_idr()` напрямую. DataChannel `{type:'pli'}` (freeze recovery) bypass'ит этот limit — обрабатывается напрямую в `datachannel.c`.
- **ABR (адаптивный битрейт)**: `webrtc_streamer.c` регистрирует `on_receiver_packet_loss_cb` через `peer_connection_on_receiver_packet_loss()`. Логика: `fraction_loss*256 > 20` → bitrate × 0.8 + IDR; `fraction_loss == 0` в течение 5 с → bitrate × 1.1; границы [500 kbps, 8 Mbps]. Динамическое обновление через `esp_h264_enc_set_bitrate((esp_h264_enc_param_handle_t)hw_ph, bps)` без пересоздания энкодера. `g_abr_bitrate` сбрасывается в 0 в `webrtc_reset()` (ленивая инициализация из `camera_get_bitrate()` на первом RR).
- **Переключение разрешения**: `camera_set_resolution(w, h)` устанавливает флаг `g_res_pending` (volatile, set from core 0, read by camera_task core 1). После доставки фрейма camera_task вызывает `_camera_reconfigure(new_w, new_h)` — полный teardown/reinit ISP + CSI + H.264 encoder на core 1. Sensor/ISP/AWB/AE handles хранятся как module-level globals (`g_sensor`, `g_isp_hdl`, `g_awb_ctlr`, `g_ae_ctlr`). Default разрешение: 800×640 (конфиг в `config.h`).
- **Reconnect (Android)**: `SignalingClient` автоматически переподключается с backoff [1с, 2с, 4с, 8с, 30с] при `onFailure`/`onClosing`/`TYPE_PEER_DISCONNECTED`. `MainActivity.onDisconnected` больше не показывает overlay — `onReconnecting` lambda показывает таймер в status bar и VR HUD. `close()` останавливает reconnect loop.
- **Freeze detection (browser)**: `stats.js` отслеживает `framesDecoded` каждую секунду; если не растёт ≥3 с при `connectionState == 'connected'` — вызывает `FPVDataChannel.send({type:'pli'})`. `WebRTCEngine.kt` делает то же через watcher coroutine (scope отменяется в `close()`) на основе `EglVideoSink.frameCount`.
- `encode_ms` в DataChannel `ts`-сообщениях — реальное время кодирования (в отличие от `streamer.html`, где ≈ 0).
- **HW H.264 формат**: ESP32-P4 rev 1.x поддерживает ТОЛЬКО `ESP_H264_RAW_FMT_O_UYY_E_VYY`. ISP выдаёт O_UYY_E_VYY **напрямую** (подтверждено hexdump: паттерн C Y Y | C Y Y, stride = W×3/2) — конвертация I420→O_UYY_E_VYY **не нужна**, frame_buf передаётся в энкодер прямо из DMA-буфера.
...
- **libpeer `onicecandidate` (ESP32-P4)**: callback `on_ice_candidate_cb(char *sdp, void *userdata)` вызывается libpeer с **полным SDP offer** (включая встроенные `a=candidate` host-кандидаты) — это не trickle ICE. Из callback вызывается `signaling_send_offer(sdp)`, а НЕ `signaling_send_ice()`. `peer_connection_loop(pc)` требует выделенного FreeRTOS task с тиком 1 мс (stack 8192, priority 6, core 0) — без него ICE, DTLS и SCTP не обрабатываются. Состояние `PEER_CONNECTION_COMPLETED` (не CONNECTED) означает готовность к отправке видео. **DataChannel**: `peer_connection_create_datachannel()` НЕ вызывается — Quest использует `negotiated=true, id=0`, что означает отсутствие DCEP DATA_CHANNEL_OPEN handshake; вызов create отправил бы лишний OPEN-пакет и нарушил протокол. Вместо этого: `sctp->onopen` callback (`on_dc_open_cb`) срабатывает ~50 мс после `PEER_CONNECTION_COMPLETED` когда SCTP-ассоциация устанавливается; в этот момент устанавливается `g_dc_sid = 0` (SCTP stream 0, offerer/DTLS-client = ESP32 использует чётные stream ID, первый = 0) и `g_dc_ready = true`. **Видеобуфер libpeer**: `CONFIG_VIDEO_BUFFER_SIZE=0` (задаётся через `add_definitions` в `managed_components/sepfy__libpeer/CMakeLists.txt`) — буферизация H.264 через кольцевой буфер ОТКЛЮЧЕНА. Первый I-фрейм (~750KB) превышает дефолтный буфер (332800 байт) → кадр дропается → Quest-декодер никогда не получает ключевой кадр → чёрный экран. С `VALUE=0` `peer_connection_send_video` вызывает `rtp_encoder_encode` напрямую из camera_task (core 1). Thread-safe: SRTP-шифрование исходящего RTP (camera_task) и DTLS-read входящего (peer_loop core 0) используют разные состояния; UDP sendto() атомарен. Также исправлен баг в libpeer: `peer_connection.c` `#else`-ветка использовала `data,bytes` вместо `buf,len`. **Encode timestamp в ts-сообщении**: `encode = capture + g_encode_ms` (timestamp «после кодирования»), чтобы `encode − capture = длительность кодирования` на стороне Quest (аналог браузерного стримера где оба поля — Date.now()-timestamps).
