...
- **Основной интерфейс** — полоска под кнопками
- **Режим "Смотреть на экране"** — плашка в верхней части видео
- **VR-режим** — HUD поверх картинки в шлеме

| Метрика | Описание |
|---------|----------|
| Сеть | Односторонняя сетевая задержка (RTT/2) |
| Декод | Среднее время декодирования кадра |
| Буфер | Задержка джиттер-буфера |
| Итого | Суммарная задержка на стороне Quest 2 |

> Итоговая цифра не включает захват камеры и энкодинг на стороне стримера — эти данные недоступны получателю без специальной синхронизации времени.
