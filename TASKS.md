...
- [x] Clock sync выполняется через 3 с после открытия канала
- [x] E2E метрика отображается в VR HUD (< 5 с после установки потока)
- [x] `clockOffset` совпадает с браузерной версией ±2 мс при параллельном тесте
- [x] При разрыве канала метрика скрывается без краша
